...
RESULTS_DIR = Path("results/exp4_pos_encoding")
...
def plot_loss_curves(histories, labels):
    """Training & validation loss curves (stability analysis)."""
    ...
    axes[0].set_title("Positional Encoding — Loss (Stability)", fontsize=13)
    axes[1].set_title("Positional Encoding — Accuracy", fontsize=13)
    plt.tight_layout()
...
def plot_accuracy_bar(pe_types, test_accs):
    fig, ax = plt.subplots(figsize=(7, 5))
    colors = ["royalblue", "seagreen", "tomato"]
    bars   = ax.bar(pe_types, test_accs, color=colors[:len(pe_types)],
    ...
    ax.set_title("Accuracy vs Positional Encoding Type", fontsize=13)
...
def plot_stability_bar(pe_types, stabilities):
    """Std of val loss over last 20 epochs — lower = more stable."""
    fig, ax = plt.subplots(figsize=(7, 5))
    colors = ["royalblue", "seagreen", "tomato"]
    bars   = ax.bar(pe_types, stabilities, color=colors[:len(pe_types)],
                    edgecolor="black", alpha=0.85)
    for bar, v in zip(bars, stabilities):
        ax.text(bar.get_x() + bar.get_width() / 2,
                bar.get_height() + max(stabilities) * 0.01,
                f"{v:.5f}", ha="center", va="bottom", fontsize=10)
    ax.set_xlabel("Positional Encoding", fontsize=12)
    ax.set_ylabel("Std of Val Loss (last 20 epochs)", fontsize=12)
    ax.set_title("Training Stability by Positional Encoding", fontsize=13)
    ax.grid(True, axis="y", alpha=0.3)
    plt.tight_layout()
    path = RESULTS_DIR / "stability_bar.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"  Saved: {path}")
...
def run():
    ...
    for pe in PE_TYPES:
        ...
        # Stability: std of val loss over last 20 epochs
        stability = float(np.std(h["val_loss"][-20:])) if len(h["val_loss"]) >= 20 else 0.0

        results[pe] = {"best_val_acc": best, "test_acc": test_acc,
                       "stability_val_loss_std": stability}
        histories.append(h); hist_labels.append(pe)
        print(f"  test_acc={test_acc:.4f}  stability(std)={stability:.5f}")

    # Plots
    plot_loss_curves(histories, hist_labels)
    plot_accuracy_bar(PE_TYPES, [results[pe]["test_acc"] for pe in PE_TYPES])
    plot_stability_bar(PE_TYPES, [results[pe]["stability_val_loss_std"] for pe in PE_TYPES])

    save_json(results, RESULTS_DIR / "results.json")

    print("\n  Summary:")
    for pe in PE_TYPES:
        print(f"    {pe:12s}  test_acc={results[pe]['test_acc']:.4f}"
              f"  stability={results[pe]['stability_val_loss_std']:.5f}")
    return results
