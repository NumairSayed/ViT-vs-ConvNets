"""
exp2_patch_size.py — Experiment 2: Effect of Patch Size
=========================================================
Objective : Study how patch size affects model performance and efficiency.
Task      : Train ViT with patch sizes 4×4, 8×8, 16×16 (all else constant).
...
RESULTS_DIR = Path("results/exp2_patch_size")
...
def plot_loss_curves(histories, labels):
    ...
    axes[0].set_title("Patch Size — Loss", fontsize=13)
    axes[1].set_title("Patch Size — Accuracy", fontsize=13)
    plt.tight_layout()
...
def plot_accuracy_bar(patch_sizes, test_accs):
    fig, ax = plt.subplots(figsize=(7, 5))
    colors = ["royalblue", "seagreen", "darkorange"]
    bars   = ax.bar([str(ps) for ps in patch_sizes], test_accs,
                    color=colors[:len(patch_sizes)], edgecolor="black", alpha=0.85)
    for bar, v in zip(bars, test_accs):
        ax.text(bar.get_x() + bar.get_width() / 2,
                bar.get_height() + 0.002,
                f"{v:.4f}", ha="center", va="bottom", fontsize=11)
    ax.set_xlabel("Patch Size", fontsize=12); ax.set_ylabel("Test Accuracy", fontsize=12)
    ax.set_title("Test Accuracy vs Patch Size", fontsize=13)
...
def plot_time_bar(patch_sizes, times_s):
    fig, ax = plt.subplots(figsize=(7, 5))
    colors = ["royalblue", "seagreen", "darkorange"]
    bars   = ax.bar([str(ps) for ps in patch_sizes], times_s,
                    color=colors[:len(patch_sizes)], edgecolor="black", alpha=0.85)
    for bar, v in zip(bars, times_s):
        ax.text(bar.get_x() + bar.get_width() / 2,
                bar.get_height() + max(times_s) * 0.01,
                f"{v/60:.1f}m", ha="center", va="bottom", fontsize=11)
    ax.set_xlabel("Patch Size", fontsize=12); ax.set_ylabel("Training Time (s)", fontsize=12)
    ax.set_title("Training Time vs Patch Size", fontsize=13)
    ax.grid(True, axis="y", alpha=0.3)
    plt.tight_layout()
    path = RESULTS_DIR / "time_bar.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"  Saved: {path}")
...
def run():
    print("\n" + "="*60)
    print("  Experiment 2: Effect of Patch Size")
    print("="*60)
    ...
    results    = {"patch_sizes": PATCH_SIZES, "test_accs": [],
                  "training_times_s": [], "num_patches": []}
    histories, labels = [], []
    ...
    for ps in PATCH_SIZES:
        ...
        print(f"  test_acc={test_acc:.4f}  time={elapsed/60:.1f}min")

    # Plots
    plot_loss_curves(histories, labels)
    plot_accuracy_bar(PATCH_SIZES, results["test_accs"])
    plot_time_bar(PATCH_SIZES, results["training_times_s"])

    save_json(results, RESULTS_DIR / "results.json")

    print("\n  Summary:")
    for ps, acc, t in zip(PATCH_SIZES, results["test_accs"], results["training_times_s"]):
        print(f"    patch={ps}  test_acc={acc:.4f}  time={t/60:.1f}min")
    return results
